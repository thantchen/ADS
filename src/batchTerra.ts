...
const lcdAddress = 'https://soju-lcd.terra.dev'
const concurrency = 20
const Q_QUEUE_NAME = 'lp:terra:queue'
...
async function batchQueue() {
  ...
  const candidateValues: Value[] = await Queue.peek(Q_QUEUE_NAME, MAX_SIGNATURES)
  ...
  const lpKey = await keystore.get(terraDB, 'LP', '12345678')
  ...
  await Queue.trim(Q_QUEUE_NAME, values.length)

  async function queueSendToUsers() {
    const inputs: InOut[] = values.map(({ denom, amount }): InOut => ({
      address: lpKey.address,
      coins: [{ denom, amount }]
    }));

    const outputs: InOut[] = values.map(({ denom, amount }, index): InOut => ({
  ...
  async function queueSendToLP() {
    ...
    inputs.push(...values.map(({ denom, amount }, index): InOut => ({
      address: keys[index].address,
      coins: [{ denom, amount }]
    })))
    ...
    outputs.push(...values.map(({ denom, amount }): InOut => ({
      address: lpKey.address,
      coins: [{ denom, amount }]
    })))
...
async function batchSend() {
  ...
  if (from === 'lp') {
    ...
    const { value: tx } = generateStdTx(
      ...
      'payment batch: lp to users'
    )

    console.log(`lp > users: ${inputs.length}`)

    tx.signatures.push(await transaction.sign(null, lpKey, tx, {
      chain_id: 'soju-0009',
  ...
  if (from === 'user') {
    ...
    const taxRate = await client.queryTaxRate({ lcdAddress })
    ...
    const accounts = await Bluebird.map(keys, ({ address }) =>
      client.queryAccount({ lcdAddress, address }), { concurrency })

    tx.signatures.push(...await Bluebird.map(keys, (key, index) =>
...
async function main() {
  // Get LP key and query account for account_number and sequence
  lpKey = await keystore.get(terraDB, 'LP', '12345678')
  lpAccount = await client.queryAccount({ lcdAddress, address: lpKey.address })
